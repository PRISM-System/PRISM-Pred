...
from db_client import DBBridge
...
# .env 로딩 뒤 (LLM_API_URL, DB_API_URL 읽은 다음) 인스턴스 생성
llm = LLMBridge(base_url=LLM_API_URL, agent="prediction-nlu")
db  = DBBridge(base_url=DB_API_URL)
...
@app.post("/api/v1/prediction/nl/parse", response_model=ParsedSpec, tags=["NL"])
def nl_parse(body: NLRequest):
    """
    사용자가 입력한 자연어를 LLM으로 구조화(JSON)해서 반환
    """
    # 스키마 힌트(선택)
    hint = """{
      "taskId": "string (if missing, generate task_YYYYMMDDhhmmss)",
      "timeRange": "YYYY-MM-DD HH:MM:SS - HH:MM:SS",
      "sensor_name": "e.g., CMP",
      "target_cols": "e.g., MOTOR_CURRENT",
      "constraints": [{"type":"time","value":"..."}],
      "userRole": "engineer|operator|..."
    }"""
    spec = llm.extract_json_from_text(body.query, schema_hint=hint)

    # 필수 보정
    import datetime as dt
    if not spec.get("taskId"):
        spec["taskId"] = "task_" + dt.datetime.now().strftime("%Y%m%d%H%M%S")
...
@app.post("/api/v1/prediction/nl/generate", response_model=NLGResponse, tags=["NL"])
def nl_generate(body: NLGRequest):
    text = llm.render_prompt_from_json(body.payload, tone=body.tone or "operator-ko")
    return NLGResponse(text=text)

# 3) DB에서 시계열 구간 조회
class DBQueryRequest(BaseModel):
    table: str = Field(..., example="process_timeseries")
    sensor_name: str = Field(..., example="CMP")
    columns: List[str] = Field(..., example=["MOTOR_CURRENT", "MOTOR_TEMP"])
    start_ts: str = Field(..., example="2025-08-20 09:00:00")
    end_ts: str = Field(..., example="2025-08-20 09:10:00")
...
@app.post("/api/v1/prediction/db/select", response_model=DBQueryResponse, tags=["DB"])
def db_select(body: DBQueryRequest):
    df = db.fetch_timeseries(
        table=body.table,
        ...
        end_ts=body.end_ts,
        ts_col=body.ts_col,
        sensor_col=body.sensor_col,
    )
    return DBQueryResponse(rows=df.to_dict(orient="records"), n_rows=int(df.shape[0]))
...
def run_direct(body: DirectRunRequest):
    ...
    # 2) 데이터 소스: (1순위) DB → (폴백) CSV
    df = None

    # --- DB 시도 ---
    if DB_API_URL:
        try:
            # timeRange 파싱 "YYYY-MM-DD HH:MM:SS - HH:MM:SS"
            tr = body.timeRange
            start_ts, end_ts = [s.strip() for s in tr.split("-")]
            # 대상 컬럼: 타깃 + 예시로 타깃만 사용 (필요 시 컨텍스트 컬럼 확장)
            cols = [body.target_cols]
            # 테이블명은 환경/팀 규약에 맞춰 조정
            df_db = db.fetch_timeseries(
                table="process_timeseries",
                sensor_name=body.sensor_name,
                columns=cols,
                start_ts=start_ts,
                end_ts=end_ts,
            ...
            if not df_db.empty:
                # CSV와 동일한 인터페이스 유지 위해 가짜 앞 4컬럼 구성
                # (run.py 파이프라인이 5번째 열부터 feature로 쓰고 있어서)
                df = df_db.copy()
                # 앞쪽 4개 메타 컬럼 더미 생성
                for col in ["meta1","meta2","meta3","meta4"]:
                    df[col] = None
                # 메타 4개 + 실제 컬럼 순서로 재정렬
                df = df[["meta1","meta2","meta3","meta4"] + list(df_db.columns)]
                _notice(events, f"DB 조회 성공: rows={df.shape[0]}, cols={df.shape[1]}")
        except Exception as e:
            _notice(events, f"DB 조회 실패: {e}")
